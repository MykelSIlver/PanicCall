...
feature: the relay itself keeps no history at all, beyond the single
pending message described below. Any message history a person sees is
built and stored entirely on their own device, from the `text`/
`text_sent`/`text_delivered` messages their client observes -- the
relay never writes message *content* to disk beyond the one pending
message needed to survive a restart. Each `text`/`text_sent`/
...
pending message per member and delivers it -- using the exact same
`{"type":"text",...}` shape as a live relay, so clients need no special
handling for the queued case -- the moment that member next completes
the `hello` handshake. Sending a second `text` while the peer is still
offline overwrites the first; only the latest matters. This pending
message **is durable**: it's written to `--pending-state` (a small
JSON file, one entry per token with a message currently waiting) on
every change, so a relay restart -- planned or a crash -- doesn't lose
it. This needs its own writable mount, separate from the read-only
